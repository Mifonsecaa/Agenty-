...
function normalizeMessages(messages) {
    return messages.filter((m)=>[
            ...
            'assistant',
            'system'
        ].includes(m.role)).map((m)=>({
            role: m.role,
            content: m.content
        }));
}
async function generateDemoReply(messages, provider, systemPrompt) {
...
async function POST(request) {
    try {
        ...
        const { messages, provider = 'openai', isDemo = false, demoContext = '', agentId, systemPrompt } = body;
        ...
        if (isDemo) {
            let demoSystemPrompt = "Eres 'AgentyBot', el asistente virtual experto de ventas para Agenty.ai. Responde corto, útil y amigable.";
            if (demoContext?.trim()) {
        ...
        if (!agentId) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'agentId es requerido en modo privado'
            }, {
                status: 400
            });
        }
        ...
        const effectiveSystemPrompt = systemPrompt?.trim() || config.systemPrompt || `Eres un asistente virtual experto para ${business.name}. Sé amable, claro y orientado a resolver.`;
        const content = await generateDemoReply(conversationMessages, provider, effectiveSystemPrompt);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            role: 'assistant',
            content
        });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c90a5a49._.js.map