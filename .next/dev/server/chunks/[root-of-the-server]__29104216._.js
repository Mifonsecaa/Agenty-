...
const aiService = {
    async generateResponse (businessId, messages, options = {}) {
        try {
            ...
            const asksForDocument = /(menu|men[uú]|cat[aá]logo|carta|pdf|archivo|documento|imagen|foto|lista\s+de\s+precios|precios\s+completos|menu\s+completo|base\s+de\s+conocimiento|conocimiento)/i.test(lastUserMessage);
            const asksToSendFile = /(env[ií]a|enviame|env[ií]ame|adjunta|adjuntame|adj[uú]ntame|p[aá]same|pasame|comp[aá]rteme|comparteme|m[aá]ndame|mandame|quiero\s+el\s+archivo|descargar)/i.test(lastUserMessage);
            ...
            if (availableFiles.length > 0) {
                systemPrompt += `\n\nTIENES ACCESO A LOS SIGUIENTES ARCHIVOS. Primero responde usando el contenido de la base de conocimiento. SOLO añade [MEDIA_URL: <url_del_archivo>] cuando el usuario pida explícitamente ENVIAR o ADJUNTAR el archivo.
            ...
            if (asksSensitiveData && !ragContext) {
            ...
            if (asksToSendFile && availableFiles.length > 0 && !/\[MEDIA_URL:\s*[^\]]+\]/i.test(finalResponse)) {
...
"[project]/app/api/knowledge/preview/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function resolveWorkbookType(fileName) {
    return /\.xlsm$/i.test(fileName) ? "xlsm" : "xlsx";
}
function sanitizeWorksheetText(value) {
    return value.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, "").trim();
}
function extractSpreadsheetText(buffer, XLSX) {
    ...
    for (const sheetName of selectedSheets){
        ...
        const lines = rows.slice(0, 220).map((row)=>row.map((cell)=>(cell ?? "").toString().trim()).join(" | ").trim()).filter(Boolean);
        if (lines.length > 0) {
            blocks.push(`[HOJA: ${sheetName}]\n${lines.join("\n")}`);
        }
    }
    return sanitizeWorksheetText(blocks.join("\n\n"));
}
function normalizeFilePath(fileUrl) {
    const normalizedFileUrl = fileUrl.trim();
    if (!normalizedFileUrl.startsWith("/uploads/")) {
        throw new Error("Archivo no soportado para previsualizacion");
    }
    const fileName = decodeURIComponent(normalizedFileUrl.split("/").pop() || "");
    if (!isSpreadsheetFile(fileName)) {
        throw new Error("Solo se pueden previsualizar archivos .xlsx y .xlsm");
    }
    const localPath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(process.cwd(), "public", normalizedFileUrl.replace(/^\//, ""));
    return {
        normalizedFileUrl,
        fileName,
        localPath
...
async function assertBusinessAccessOrThrow(businessId, email) {
    const business = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].business.findFirst({
        where: {
            id: businessId,
            user: {
...
function mapSheetPreview(sheet, XLSX) {
    const rows = XLSX.utils.sheet_to_json(sheet, {
        header: 1,
        raw: false,
        blankrows: false
    ...
    const bodyRows = rows.slice(1, 101).map((row)=>row.map((cell)=>(cell ?? "").toString()));
    return {
...
async function GET(req) {
    try {
        ...
        if (!businessId || !fileUrl) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: "businessId y fileUrl son requeridos"
            }, {
                status: 400
            });
        }
        await assertBusinessAccessOrThrow(businessId, session.user.email);
        const { normalizedFileUrl, fileName, localPath } = normalizeFilePath(fileUrl);
        ...
        const stats = await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["stat"])(localPath);
        ...
        const status = message === "Business not found or unauthorized" ? 404 : message.includes("previsualizacion") || message.includes(".xlsx") ? 400 : 500;
        console.error("[Knowledge Preview] Error:", error);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: message === "Internal Error" ? "Internal Error" : message
        }, {
            status
        });
    }
...
async function PATCH(req) {
    try {
        ...
        if (!businessId || !fileUrl || edits.length === 0) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: "businessId, fileUrl y edits son requeridos"
            }, {
                status: 400
            });
        }
        if (edits.length > MAX_EDIT_OPERATIONS) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: `Demasiadas ediciones en una sola solicitud (max ${MAX_EDIT_OPERATIONS})`
            }, {
                status: 400
            });
        }
        ...
        const { normalizedFileUrl, fileName, localPath } = normalizeFilePath(fileUrl);
        const XLSX = __turbopack_context__.r("[project]/node_modules/xlsx/xlsx.mjs [app-route] (ecmascript)");
        const originalBuffer = await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["readFile"])(localPath);
        const workbook = XLSX.read(originalBuffer, {
            type: "buffer"
        });
        for (const edit of edits){
            const sheetName = typeof edit?.sheetName === "string" ? edit.sheetName : "";
            const rowIndex = Number(edit?.rowIndex);
            const colIndex = Number(edit?.colIndex);
            const value = edit?.value == null ? "" : String(edit.value);
            if (!sheetName || !Number.isInteger(rowIndex) || !Number.isInteger(colIndex) || rowIndex < 0 || colIndex < 0) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: "Formato de edit invalido"
                }, {
                    status: 400
                });
            }
            if (value.length > 2000) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: "El valor de celda excede el limite de 2000 caracteres"
                }, {
                    status: 400
                });
            }
            ...
            currentRange.e.r = Math.max(currentRange.e.r, rowIndex);
            currentRange.e.c = Math.max(currentRange.e.c, colIndex);
            sheet["!ref"] = XLSX.utils.encode_range(currentRange);
        }
        const outputBuffer = XLSX.write(workbook, {
            type: "buffer",
            bookType: resolveWorkbookType(fileName)
        });
        await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["writeFile"])(localPath, outputBuffer);
        const extractedText = extractSpreadsheetText(outputBuffer, XLSX);
        if (!extractedText) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: "No se pudo extraer texto del archivo actualizado"
            }, {
                status: 422
            });
        }
        await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].$executeRawUnsafe(`DELETE FROM "KnowledgeItem" WHERE "businessId" = $1 AND metadata->>'fileUrl' = $2`, businessId, normalizedFileUrl);
        const enqueue = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$rag$2f$queue$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["enqueueKnowledgeIngestion"])({
            ...
            metadata: {
                fileName,
                fileType: /\.xlsm$/i.test(fileName) ? "application/vnd.ms-excel.sheet.macroEnabled.12" : "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                fileUrl: normalizedFileUrl,
                source: "excel_editor",
                sourceId: normalizedFileUrl
            }
        });
        if (enqueue.missingQueueTable) {
            await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$rag$2f$ingestion$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["ingestionService"].ingestText(businessId, `[EXCEL: ${fileName}]\n${extractedText}`, {
                fileName,
                fileType: /\.xlsm$/i.test(fileName) ? "application/vnd.ms-excel.sheet.macroEnabled.12" : "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                fileUrl: normalizedFileUrl,
                source: "excel_editor",
            ...
            void (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$rag$2f$queue$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["processKnowledgeQueueBatch"])(1).catch((error)=>{
                console.error("[Knowledge Preview PATCH] Queue kickoff error:", error);
            });
        }
        const updatedStats = await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["stat"])(localPath);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: true,
            ...
            data: {
                ...
                reindexed: true,
                queued: !enqueue.missingQueueTable,
                jobId: enqueue.job?.id || null
            }
        });
    } catch (error) {
        const message = error instanceof Error ? error.message : "Internal Error";
        const status = message === "Business not found or unauthorized" ? 404 : message.includes("previsualizacion") || message.includes(".xlsx") || message.includes(".xlsm") ? 400 : 500;
        console.error("[Knowledge Preview PATCH] Error:", error);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: message === "Internal Error" ? "Internal Error" : message
        }, {
            status
...
//# sourceMappingURL=%5Broot-of-the-server%5D__29104216._.js.map