...
async function describeImageForKnowledge(buffer, mimeType) {
...
function extractSpreadsheetText(buffer) {
    ...
    for (const sheetName of selectedSheets){
        ...
        const lines = limitedRows.map((row)=>row.map((cell)=>(cell ?? "").toString().trim()).join(" | ").trim()).filter(Boolean);
        if (lines.length > 0) {
...
async function POST(req) {
    try {
        ...
        const isTextLike = safeType.startsWith("text/") || [
            "application/json",
            "text/csv"
        ].includes(safeType) || /\.(txt|md|csv|json)$/i.test(name || "");
        const isSpreadsheet = [
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            "application/vnd.ms-excel.sheet.macroEnabled.12"
        ].includes(safeType) || /\.(xlsx|xlsm)$/i.test(name || "");
        if (!url && name) {
            uploadedBuffer = isTextLike ? Buffer.from(text || "", "utf-8") : Buffer.from(text || "", "base64");
            if (uploadedBuffer.byteLength > 0) {
                ...
                const outputPath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(uploadDir, uniqueName);
                await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["writeFile"])(outputPath, uploadedBuffer);
        ...
        if (safeType === "application/pdf") {
            ...
            if (!uploadedBuffer || uploadedBuffer.byteLength === 0) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: "La imagen no contiene datos para procesar"
                }, {
                    status: 400
                });
            }
            ...
            text = extractedText ? `[EXCEL: ${name || "archivo"}]\n${extractedText}` : `[EXCEL: ${name || "archivo"}] Archivo subido sin celdas legibles.`;
        } else if (isTextLike) {
            text = text || "";
        } else {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1a68ed39._.js.map