...
async function sendTelegramMedia({ botToken, chatId, mediaUrl, caption }) {
    ...
    const data = await res.json();
    if (!data.ok) {
        console.error("[Telegram Sender] Media error:", data.description);
        return {
            ok: false,
            error: data.description
        };
...
async function POST(req) {
    try {
        ...
        const aiReply = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$ai$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["aiService"].generateResponse(business.id, [
            {
                role: "user",
                content: messageText
            }
        ...
        let mediaUrl = null;
        const mediaMatch = replyText.match(/\[MEDIA_URL:\s*(.*?)\]/);
        if (mediaMatch?.[1]) {
            mediaUrl = mediaMatch[1].trim();
            replyText = replyText.replace(/\[MEDIA_URL:\s*.*?\]/, "").trim();
        ...
        if (mediaUrl) {
            await (0, __TURBOPACK__imported__module__$5b$project$5d2f$services$2f$telegram$2d$sender$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sendTelegramMedia"])({
                botToken: TELEGRAM_BOT_TOKEN,
                chatId: numericChatId,
                mediaUrl,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__319fe97b._.js.map